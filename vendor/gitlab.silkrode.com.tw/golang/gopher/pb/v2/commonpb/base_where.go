...
func (where *BaseWhere) WhereWith(table string) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		if where.IDs != nil && len(where.IDs) != 0 {
			stmt := fmt.Sprintf("%s.id IN (?)", table)
			db = db.Where(stmt, where.IDs)
		}
		if where.SearchIn != "" && where.Keyword != "" {
			fields := strings.Split(where.SearchIn, ",")
			for _, field := range fields {
				field := field
				db = db.Where(fmt.Sprintf("%s.%s like ?", table, field), "%"+where.Keyword+"%")
			}
		}
		if where.CreatedAtLt != nil && *where.CreatedAtLt != zeroTime {
			sec := (*where.CreatedAtLt).Unix()
			start := time.Unix(int64(sec), 0).UTC()
			stmt := fmt.Sprintf("%s.created_at < ?", table)
			db = db.Where(stmt, start)
		}
		...
		if where.CreatorID > 0 {
			stmt := fmt.Sprintf("%s.creator_id = ?", table)
			db = db.Where(stmt, where.CreatorID)
		}
		if where.CreatorName != "" {
			stmt := fmt.Sprintf("%s.creator_name = ?", table)
			db = db.Where(stmt, where.CreatorName)
		}
		if where.UpdaterID > 0 {
			stmt := fmt.Sprintf("%s.updater_id = ?", table)
			db = db.Where(stmt, where.UpdaterID)
		}
		if where.UpdaterName != "" {
			stmt := fmt.Sprintf("%s.updater_name = ?", table)
			db = db.Where(stmt, where.UpdaterName)
		}
		if where.DeletedAtLt != nil && *where.DeletedAtLt != zeroTime {
			sec := (*where.DeletedAtLt).Unix()
			start := time.Unix(int64(sec), 0).UTC()
			stmt := fmt.Sprintf("%s.deleted_at < ?", table)
			db = db.Where(stmt, start)
		}
		...
		if where.ExpiredAtGt != nil && *where.ExpiredAtGt != zeroTime {
			...
			start := time.Unix(int64(sec), 0).UTC()
			stmt := fmt.Sprintf("%s.expired_at > ?", table)
			db = db.Where(stmt, start)
		}
		...
		if where.CompletedAtGt != nil && *where.CompletedAtGt != zeroTime {
			...
			start := time.Unix(int64(sec), 0).UTC()
			stmt := fmt.Sprintf("%s.completed_at > ?", table)
			db = db.Where(stmt, start)
		}
		...
		if where.TradeTimeGt != nil && *where.TradeTimeGt != zeroTime {
			...
			start := time.Unix(int64(sec), 0).UTC()
			stmt := fmt.Sprintf("%s.trade_time > ?", table)
			db = db.Where(stmt, start)
		}
